...
// User enters MonCash/NatCash transaction ID after paying.
// Backend validates it (matches an SMS we received) and grants access.
...
const MONCASH_NUMBER = "+509 XXXX-XXXX"; // REPLACE WITH YOUR NUMBER
const NATCASH_NUMBER = "+509 XXXX-XXXX"; // REPLACE WITH YOUR NUMBER
...
const PLANS = [
  {
    ...
    features: [
      "15 scans par jour",
      "50 messages au prof par jour",
      "5 schémas au tableau par jour",
      "20 quiz par jour",
...
export default function Paywall({ onSuccess }) {
  ...
  const [step, setStep] = useState("choose"); // choose | pay | validate
  ...
  const validateTransaction = async () => {
    ...
    try {
      const response = await fetch(
        `/api/payment-webhook?txId=${encodeURIComponent(txId.trim())}`
      );
      const data = await response.json();

      if (response.status === 404) {
        setError(
          "Transaction non trouvée. Vérifie l'ID ou attends quelques minutes (les SMS prennent parfois du temps)."
        );
        return;
      }
      ...
      // Success! Upgrade the plan
      const plan = data.data.plan;
      if (plan !== selectedPlan?.id) {
        setError(
          `Le montant payé (${data.data.amount} HTG) correspond au plan ${plan}, pas ${selectedPlan?.name}.`
        );
        return;
      }
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-28">
        <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 py-3 sticky top-0 z-10">
          <button onClick={() => setStep("choose")} className="text-sm text-violet-600 font-semibold">
            ← Retour
          </button>
        </header>
        ...
        <div className="px-4 py-6 max-w-md mx-auto">
          ...
          <p className="text-sm text-slate-500 mb-6">
            Envoie <b>{selectedPlan.price} HTG</b> à l'un de ces numéros
          </p>
          ...
          <div className="rounded-2xl bg-white dark:bg-slate-900 p-4 shadow-sm mb-3">
            ...
            <div className="flex items-center gap-2 mb-2">
              <code className="flex-1 text-lg font-mono font-bold text-slate-900 dark:text-white">
                {MONCASH_NUMBER}
              </code>
              <button
                onClick={() => copyNumber(MONCASH_NUMBER)}
          ...
          <div className="rounded-2xl bg-white dark:bg-slate-900 p-4 shadow-sm mb-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => copyNumber(NATCASH_NUMBER)}
                className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1"
              >
                <Copy size={12} />
                Copier
          ...
          <div className="rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-500/30 p-4 mb-6">
            <div className="flex items-start gap-2 text-xs text-amber-800 dark:text-amber-300">
              ...
              <div>
                ...
                Après avoir payé, tu vas recevoir un SMS de confirmation avec un <b>ID de transaction</b>. Colle cet ID ci-dessous pour activer ton plan.
              </div>
            </div>
          </div>
