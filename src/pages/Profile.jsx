...
// User profile: edit name, change tutor personality/language, view plan, toggle theme.

import { useState } from "react";
import { motion } from "framer-motion";
import {
  User, Edit2, Check, X, Settings, Crown, Zap,
  Moon, Sun, LogOut, Sparkles, RotateCcw,
} from "lucide-react";
        ...
        <div className="relative flex flex-col items-center">
          ...
          {editingName ? (
            <div className="flex items-center gap-2">
              ...
              />
              <button onClick={saveName} className="p-1.5 rounded-lg bg-emerald-500">
                <Check size={16} />
              </button>
              <button onClick={() => { setEditingName(false); setNameInput(preferences?.name || ""); }} className="p-1.5 rounded-lg bg-red-500">
                <X size={16} />
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setEditingName(true)} className="p-1.5 rounded-lg bg-white/20 hover:bg-white/30">
                <Edit2 size={14} />
              </button>
            </div>
          )}
        ...
        <Section icon={<Settings size={18} className="text-slate-600" />} title="Paramètres">
          <div className="rounded-2xl bg-white dark:bg-slate-800 shadow-sm divide-y divide-slate-100 dark:divide-slate-700">
            ...
            <button
              ...
              className="w-full p-4 flex items-center gap-3 text-left text-red-600 dark:text-red-400"
            >
              <RotateCcw size={18} />
              <div className="flex-1">
