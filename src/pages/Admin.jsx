// src/pages/Admin.jsx
// Hidden /admin page. Upload past exams text → AI generates 50 MCQs per subject.

import { useState, useEffect } from "react";
import { motion } from "framer-motion";
import {
  Lock, RefreshCw, CheckCircle2, XCircle,
  Loader2, Database, FileText,
} from "lucide-react";
import {
...
export default function Admin() {
  ...
  useEffect(() => {
    if (authed) setCachedSubjects(getAllCachedSubjects());
  }, [authed, result]);
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Admin Laureat AI</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Entre ton token administrateur.</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20">
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 py-4">
        ...
        <p className="text-xs text-slate-500 mt-0.5">Gestion des quiz et examens</p>
      </header>

      <section className="px-4 mt-6">
        <h2 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Quiz en cache</h2>
        ...
        {cachedSubjects.length > 0 && (
          <button onClick={() => { clearQuizCache(); setCachedSubjects([]); }} className="mt-3 text-xs text-red-500 underline">
            Vider le cache
          </button>
        )}
      </section>
      ...
      <section className="px-4 mt-8">
        <h2 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Générer un quiz</h2>
        <div className="rounded-2xl bg-white dark:bg-slate-900 p-4 shadow-sm space-y-4">
          ...
          <div>
            ...
            <textarea value={pastExamsText} onChange={(e) => setPastExamsText(e.target.value)}
              placeholder="Colle ici le texte des examens MENFP 2023, 2024, 2025..."
              rows={8}
          ...
          {result && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 text-xs text-emerald-700 dark:text-emerald-300">
              ...
              <span>{result.count} questions générées pour {result.subject}. Expire le {formatDate(result.expiresAt)}.</span>
            </div>
          )}
        </div>
      </section>
