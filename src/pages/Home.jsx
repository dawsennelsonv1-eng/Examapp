// src/pages/Home.jsx
// Personalized home with countdown, missions, leaderboard.
// French-only UI. Uses preferences.name for personal greeting.
// Vivid colors (not washed-out): solid darks, strong contrasts.

import { motion } from "framer-motion";
import { useNavigate } from "react-router-dom";
import {
  Calculator, BookOpen, Edit3, ChevronRight, Trophy, Flame,
  Scan, Sparkles, Target,
} from "lucide-react";
...
export default function Home() {
  ...
  const missions = [
    {
      icon: Calculator,
      title: "Maîtrise 2 formules",
      subtitle: "Physique - Chapitre 5",
      count: 2,
    ...
    {
      icon: BookOpen,
      title: "Lis 1 texte d'histoire",
      subtitle: "Sciences Sociales",
      count: 1,
    ...
    {
      icon: Edit3,
      title: "Fais 5 questions de quiz",
  ...
  const leaderboard = [
    ...
    { rank: 3, name: name, score: 82, you: true },
      ...
      <header className="relative bg-gradient-to-br from-violet-700 via-indigo-700 to-slate-900 text-white p-6 pb-12 rounded-b-3xl shadow-2xl shadow-violet-500/30 overflow-hidden">
        ...
        >
          ...
          <div className="flex items-baseline gap-2 mb-1">
            ...
            <div className="text-sm font-semibold text-white/90 pb-1">
              jours
            </div>
          </div>
          <p className="text-sm text-white/70">
            avant ton examen MENFP
          </p>
        ...
        <section>
          ...
          <div className="rounded-2xl bg-white dark:bg-slate-800 shadow-md shadow-slate-200/50 dark:shadow-slate-900/50 overflow-hidden ring-1 ring-slate-100 dark:ring-slate-700">
            {leaderboard.map((l) => (
              <div
                key={l.rank}
                className={`flex items-center gap-4 px-4 py-3 border-b last:border-b-0 border-slate-100 dark:border-slate-700 ${
              ...
              >
                <div
                  className={`w-9 h-9 rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0 ${
                    l.rank === 1
                      ? "bg-gradient-to-br from-amber-400 to-yellow-500 text-amber-900 shadow-md shadow-amber-500/30"
                      : l.rank === 2
                      ? "bg-gradient-to-br from-slate-300 to-slate-400 text-slate-700"
                      : l.rank === 3
                      ? "bg-gradient-to-br from-orange-400 to-amber-500 text-orange-900 shadow-md shadow-orange-500/30"
