...
// Quiz feature using AI-generated questions from the admin-uploaded past exams.
...
export default function Quizzes() {
  ...
  const startQuiz = (subject) => {
    ...
    // Shuffle and take 10 random questions per session
      ...
      <div className="pb-28">
        <div className="px-4 py-6">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Questions inspirées des examens MENFP des 3 dernières années
          </p>
        </div>
            ...
            {subjects.map((subject, i) => {
              ...
              return (
                <motion.button
                  ...
                  disabled={!available}
                  className={`w-full flex items-center gap-3 p-4 rounded-xl bg-white dark:bg-slate-800 shadow-sm text-left ${
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-28">
        <header className="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            <button onClick={restart} className="text-xs font-semibold text-violet-600">
              Quitter
            </button>
            <div className="text-xs text-slate-500">
        ...
        <div className="px-4 mt-6">
          <div className="mb-4">
            <div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-1">
              {selectedSubject} · {q.askedIn?.year || ""} {q.askedIn?.session || ""}
            </div>
              ...
              return (
                <motion.button
                  ...
                  className={`w-full text-left p-4 rounded-xl border-2 transition-colors ${
                    showAsCorrect
                      ...
                      : isSelected
                      ? "bg-violet-50 dark:bg-violet-950/30 border-violet-500 text-violet-900 dark:text-violet-200"
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm leading-relaxed flex-1">{opt}</div>
          ...
          <AnimatePresence>
            {showExplanation && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="mt-4 p-4 rounded-xl bg-slate-100 dark:bg-slate-800"
              >
                <div className="text-xs uppercase font-bold text-slate-500 mb-2">
                  Explication
                </div>
                <p className="text-sm text-slate-900 dark:text-white leading-relaxed mb-3">
                  {q.explanation}
                </p>
                {q.trap && (
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-28">
      ...
      <div className="px-4 space-y-3">
        <button
          ...
          className="w-full py-3 rounded-xl bg-violet-600 text-white font-bold shadow-lg flex items-center justify-center gap-2"
        >
          <RefreshCw size={16} />
          Refaire le quiz
        </button>
        <button
          onClick={restart}
          className="w-full py-3 rounded-xl bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-white font-semibold"
        >
